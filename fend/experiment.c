...
void prt(struct sandbox* sandb, struct user_regs_struct *regs) {
  if(regs->orig_rax == -1) {
    printf("[SANDBOX] Segfault ?! KILLING !!!\n");
  } else {
    printf("[%d/%d] calling syscall (%lu)\n", pid, ppid, regs->orig_rax);
  }
}
...
void sandb_handle_syscall(struct sandbox *sandb) {
  ...
  if(regs.orig_rax == -1) {
    ...
    printf("[%d/%d] Trying to use devil syscall (%lu) ?!? KILLING !!!\n",
	   pid, ppid, regs.orig_rax);
  }
  //sandb_kill(sandb);
}
